...
# %%
fo = pd.read_hdf("fo.hdf", key="fo")
print(list(fo["forecast_id"].drop_duplicates()))

fous = [fo]
# fous = []
for fid in [f for f in ff.index if f not in list(fo["forecast_id"].drop_duplicates())]:
    # for fid in [f for f in ff.index[:3]]:
    print(fid)
    dfs = []
    ...
    idx = df[df["forecast_id"] == fid].index
...
fo.to_hdf("fo.hdf", key="fo", mode="w")
# %%
hist, dd = get_latest_history(start=pd.Timestamp("2024-07-01", tz="GB"))
# %%
ph = ph.set_index("date_time")[["agile", "day_ahead"]]
ph.index = pd.to_datetime(ph.index)
# %%%
ag = get_agile(start=ph.index[-1])
da = day_ahead_to_agile(ag, reverse=True)
ph = pd.concat([ph, pd.concat([da, ag], axis=1)])
# %%
...
# ax.plot(hist['demand'].loc['2024-11'], lw=2, zorder=3, color='black')

# hist[cols].loc['2024-11'].plot.area(stacked=True,figsize=(16,6), alpha=0.5, ax=ax, zorder=0)

start = "2024-11-01"
end = "2024-11-08"
dd[2][cols].astype(float).loc[start:].shift(3).plot.area(stacked=True, alpha=0.5, ax=ax, zorder=0)
dd[1].loc[start:].shift(0).plot(lw=2, zorder=1, color="black", ax=ax)
dd[3].loc[start:].shift(0).plot(lw=2, zorder=1, color="blue", ax=ax)
q = df.sort_index().sort_values("forecast_id").groupby(level=0).last()
q["demand"].loc[start:end].plot(ax=ax)
# dd[0].loc[start:end][['TSD', 'ND']].plot(lw=2, zorder=1, color='blue',ax=ax)

ax2 = ax.twinx()
ax2.set_ylim(-150, 250)
ax.set_ylim(0, 80000)
ph["day_ahead"].loc[start:].plot(ax=ax2)

irish = ["INTEW", "INTGRNL", "INTIRL"]
ints = [x for x in fo.columns if (("INT" in x) and (x not in irish))]
fo["ints"] = fo[ints].sum(axis=1)
foq = fo.sort_index().sort_values("forecast_id").groupby(level=0).last()
foq["ints"].loc[start:end].plot(ax=ax, lw=2)
# %%
...
dd[2][cols].astype(float).loc[start:end].shift(3).plot.area(stacked=True, figsize=(16, 6), alpha=0.5, ax=ax, zorder=0)
dd[1].loc[start:end].shift(0).plot(lw=2, zorder=1, color="black", ax=ax)
dd[3].loc[start:end].shift(0).plot(lw=2, zorder=1, color="blue", ax=ax)
# dd[0].loc[start:end][['TSD', 'ND']].plot(lw=2, zorder=1, color='blue',ax=ax)


ax2 = ax.twinx()
ph["day_ahead"].loc[start:end].plot(ax=ax2)
ax.get_legend().remove()
fig.legend(loc="center right")
# %%
...
class_model = xg.XGBClassifier(use_label_encoder=False, scale_pos_weight=0.01, eval_metric="logloss")
...
for i, c in enumerate(cols):
    ...
    xg_model.fit(train_X, train_y, sample_weight=sample_weights)
    ...
    # pred_y2 = pd.Series(data=xg_2_model.predict(pred_X), index=pred_X.index).loc[:"2024-11-05"]
    ax[0].plot(pred_y)
    # ax[0].plot(pred_y2, label=f"{nsub:0.1f} {reg} high")
    ax[1].plot(pred_class)
    ax[0].legend()
    ax[0].set_title(t)
